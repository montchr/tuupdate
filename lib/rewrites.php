 ...
 *
 * @link https://github.com/retlehs/exai/blob/master/doc/rewrites.md
 */
function exai_add_rewrites($content) {
	global $wp_rewrite;
	$exai_new_non_wp_rules = array(
		'assets/(.*)'          => THEME_PATH . '/assets/$1',
		'plugins/(.*)'         => RELATIVE_PLUGIN_PATH . '/$1'
	);
	$wp_rewrite->non_wp_rules = array_merge($wp_rewrite->non_wp_rules, $exai_new_non_wp_rules);
	return $content;
}

function exai_clean_urls($content) {
...
if (!is_multisite() && !is_child_theme()) {
	if (current_theme_supports('rewrites')) {
		add_action('generate_rewrite_rules', 'exai_add_rewrites');
	}
	...
	if (!is_admin() && current_theme_supports('rewrites')) {
		...
		add_filters($tags, 'exai_clean_urls');
	}
}
